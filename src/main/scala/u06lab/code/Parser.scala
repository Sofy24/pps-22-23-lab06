...
object Parsers:
  extension (s: String)
    def charParser(): Parser[Char] = BasicParser(s.toCharArray.toSet)
...
trait NonEmpty[T] extends Parser[T]:
  ...
  abstract override def parse(t: T): Boolean =
    empty = false;
    super.parse(t) // who is super?? => an instance of BasicParser
...
trait NotTwoConsecutive[T] extends Parser[T]:
  ...
  abstract override def parse(t: T): Boolean =
    last match
      case Some(prev) if prev == t => false // two consecutive equals
      case _ =>
        last = Some(t)
        super.parse(t)
